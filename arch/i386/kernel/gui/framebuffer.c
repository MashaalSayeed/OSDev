...
void update_framebuffer_region(rect_t rect) {
    ...
    if (rect.x + rect.w > fb->width) rect.w = fb->width - rect.x;
    if (rect.y + rect.h > fb->height) rect.h = fb->height - rect.y;

    uint8_t *fb_ptr = (uint8_t *)fb->addr;
    uint8_t *bb_ptr = (uint8_t *)backbuffer;

    for (uint32_t row = 0; row < rect.h; row++) {
        size_t fb_offset = ((rect.y + row) * fb->pitch) + (rect.x * (fb->bpp / 8));
        size_t bb_offset = ((rect.y + row) * fb->width + rect.x) * (fb->bpp / 8);

        memcpy(fb_ptr + fb_offset, bb_ptr + bb_offset, rect.w * (fb->bpp / 8));
    }
}
...
void fb_put_pixel(uint32_t x, uint32_t y, uint32_t color) {
    if (x >= fb->width || y >= fb->height) return; // Out of bounds
...
void fill_fb_area(rect_t rect, uint32_t color) {
    ...
    for (uint32_t y = 0; y < rect.h; y++) {
        for (uint32_t x = 0; x < rect.w; x++) {
            backbuffer[(rect.y + y) * fb->width + (rect.x + x)] = color;
        }
    }
}
...
void fill_fb(uint32_t color) {
    if (!backbuffer) return;
    for (uint32_t i = 0; i < fb->width * fb->height; i++) backbuffer[i] = color;
}
...
//             if (x + win->rect.x < fb->width && y + win->rect.y < fb->height) {
//                 backbuffer[(y + win->rect.y) * fb->width + (x + win->rect.x)] = color;
//             }
//         }
//     }
// }
