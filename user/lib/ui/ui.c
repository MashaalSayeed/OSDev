/*
 * ui.c — User-space UI drawing library implementation.
 *
 * Provides an embedded 8×8 bitmap font (ASCII 32–126) and a set of
 * simple drawing primitives that operate on a flat 32-bit ARGB pixel
 * buffer.  Both the compositor and individual GUI apps (e.g. notepad)
 * link against this file via the userland lib/ build.
 */
                  ...
                  int x, int y, int w, int h, uint32_t color)
{
    ...
    for (int row = 0; row < h; row++) {
        uint32_t *p = pixels + (y + row) * pitch + x;
                   ...
                   uint32_t fg, uint32_t bg, int transparent_bg)
{
    ...
    int cx = x;
    while (*s) {
        ui_draw_char(pixels, pitch, cw, ch, cx, y, *s, fg, bg, transparent_bg);
        cx += UI_FONT_W;
        s++;
    }
    return cx;
