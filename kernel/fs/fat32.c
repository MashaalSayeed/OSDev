...
static int fat32_truncate(vfs_inode_t *inode, uint32_t new_size);
...
int fat32_read_cluster(fat32_superblock_t *sb, uint32_t cluster_number, void *buffer) {
    if (cluster_number < 2) {
        kprintf(ERROR, "fat32_read_cluster: invalid cluster %d\n", cluster_number);
        return -1;
    }

    uint32_t first_sector = sb->data_sector + (cluster_number - 2) * sb->sectors_per_cluster;
    if (first_sector >= sb->total_sectors) {
        kprintf(ERROR, "fat32_read_cluster: cluster %d maps to sector %d beyond volume (%d sectors)\n",
                cluster_number, first_sector, sb->total_sectors);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < sb->sectors_per_cluster; i++) {
        if (sb->device->read_blocks(sb->device, first_sector + i, 1,
...
int fat32_write_cluster(fat32_superblock_t *sb, uint32_t cluster_number, const void *buffer) {
    uint32_t total_clusters = (sb->total_sectors - sb->data_sector) / sb->sectors_per_cluster + 2;
    if (cluster_number < 2 || cluster_number >= total_clusters) {
        printf("[fat32_write_cluster] Error: Invalid cluster number: %d\n", cluster_number);
        return -1;
    }
    ...
    if (sb->device->write_blocks) {
        if (sb->device->write_blocks(sb->device, first_sector, sb->sectors_per_cluster, buffer) != 0) {
            printf("Error: Failed to write cluster at sector %d\n", first_sector);
            return -1;
        }
        return 0;
    }
    ...
    for (uint32_t i = 0; i < sb->sectors_per_cluster; i++) {
        if (sb->device->write_blocks(sb->device, first_sector + i, 1,
...
static uint32_t fat32_allocate_cluster(fat32_superblock_t *sb) {
    ...
    for (uint32_t i = 0; i < sector_per_fat; i++) {
        if (sb->device->read_blocks(sb->device, sb->reserved_sectors + i, 1, fat_buffer) != 0) {
        ...
        for (uint32_t j = 0; j < fat_entries_per_sector; j++) {
            ...
            if (fat_entries[j] == FAT32_CLUSTER_FREE) {
                ...
                if (sb->device->write_blocks(sb->device, sb->reserved_sectors + i, 1, fat_buffer) != 0) {
...
static uint32_t fat32_get_next_cluster(fat32_superblock_t *sb, uint32_t cluster) {
    ...
    if (sb->device->read_blocks(sb->device, fat_sector, 1, buffer) != 0) {
...
static uint32_t fat32_set_next_cluster(fat32_superblock_t *sb, uint32_t cluster, uint32_t next_cluster) {
    ...
    uint8_t *buffer = kmalloc(sb->bytes_per_sector);
    if (!buffer) {
        printf("Error: Failed to allocate buffer for FAT sector %d\n", fat_sector);
        return -1;
    }
    if (sb->device->read_blocks(sb->device, fat_sector, 1, buffer) != 0) {
        printf("Error: Failed to read FAT sector %d\n", fat_sector);
    ...
    *(uint32_t *)(buffer + offset) = next_cluster & 0x0FFFFFFF;
    if (sb->device->write_blocks(sb->device, fat_sector, 1, buffer) != 0) {
...
static uint32_t fat32_read(vfs_file_t *file, void *buf, size_t count) {
    ...
    uint8_t *buffer = kmalloc(sb->cluster_size);
    if (!buffer) {
        kprintf(ERROR, "fat32_read: failed to allocate cluster buffer\n");
        return 0;
    }
    ...
    while (read < count && cluster != FAT32_CLUSTER_LAST) {
        ...
        // kprintf(DEBUG, "fat32_read: cluster=%d fat_entry=%x offset=%d\n", cluster, buffer[cluster], offset);
        if (fat32_read_cluster(sb, cluster, buffer) != 0) {
            kfree(buffer);
            return read;
        }
...
static int fat32_truncate(vfs_inode_t *inode, uint32_t new_size) {
    fat32_inode_t *fat_inode = (fat32_inode_t*)inode->fs_data;
    fat32_superblock_t *sb = (fat32_superblock_t*)inode->superblock->fs_data;
    uint32_t cluster = fat_inode->cluster;

    if (new_size >= fat_inode->size) return 0;

    if (new_size == 0) {
        uint32_t next = fat32_get_next_cluster(sb, cluster);
        if (next != FAT32_CLUSTER_LAST && next != FAT32_CLUSTER_BAD && next != FAT32_CLUSTER_FREE) {
            fat32_free_cluster_chain(sb, next);
        }
        fat32_set_next_cluster(sb, cluster, FAT32_CLUSTER_LAST);
    } else {
        uint32_t needed = (new_size + sb->cluster_size - 1) / sb->cluster_size;
        if (needed == 0) needed = 1;

        for (uint32_t i = 0; i < needed - 1; i++) {
            uint32_t next = fat32_get_next_cluster(sb, cluster);
            if (next == FAT32_CLUSTER_LAST || next == FAT32_CLUSTER_BAD) break;
            cluster = next;
        }
    ...
    fat_inode->size = new_size;
    inode->size = new_size;

    fat32_dir_entry_t entry;
    if (fat32_read_dir_entry(sb, fat_inode, &entry) == 0) {
        entry.size = new_size;
        fat32_write_dir_entry(sb, fat_inode, &entry);
    }
...
vfs_superblock_t * fat32_mount(const char *device) {
    ...
    uint8_t buffer[512];
    if (bd->read_blocks(bd, 0, 1, buffer) != 0) {
